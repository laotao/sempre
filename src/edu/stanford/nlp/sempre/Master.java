...
//		System.out.println(CharEncUtils.getCharEncoding());
		if ("GBK".equalsIgnoreCase(encoding))
			CharEncUtils.setCharEncoding(encoding);
//		System.out.println(CharEncUtils.getCharEncoding());
		...
		if (!Strings.isNullOrEmpty(opts.logPath)) {
			...
			out.println(Joiner
					...
					.join(Lists.newArrayList(
							new Date().toString(),
							session.id,
							session.lastRemoteAddr != null ? session.lastRemoteAddr
									: "(none)", line, summaryString(response))));
			out.close();
		}
	...
	private void handleQuery(Session session, String query, Response response) {
		...
		if (slashIndex != -1) {
			// if query = "where was obama born? /place_of_birth", that
			// constrains
			// derivations to only ones whose formula contains place_of_birth.
			b.setDerivConstraint(new DerivationConstraint(query
	...
	private void handleCommandHelper(Session session, String line,
			Response response) {
		...
		if (command == null || command.equals("help")) {
			...
			Executor.Response execResponse = builder.executor.execute(Formulas
	...
	public static OptionsParser getOptionsParser() {
		...
		parser.registerAll(new Object[] { "Master", Master.opts, "Builder",
				Builder.opts, "Grammar", Grammar.opts, "Derivation",
				Derivation.opts, "Parser", Parser.opts, "BeamParser",
				BeamParser.opts, "SparqlExecutor", SparqlExecutor.opts,
				"Dataset", Dataset.opts, "Params", Params.opts, "Learner",
				Learner.opts, "SemanticFn", SemanticFn.opts, "LexiconFn",
				LexiconFn.opts, "SelectFn", SelectFn.opts, "MergeFn",
				MergeFn.opts, "JoinFn", JoinFn.opts, "DescriptionValue",
				DescriptionValue.opts, "FeatureExtractor",
				FeatureExtractor.opts, "LanguageInfo", LanguageInfo.opts,
				"EntityLexicon", EntityLexicon.opts, "BinaryLexicon",
				BinaryLexicon.opts, "UnaryLexicon", UnaryLexicon.opts,
				"FreebaseInfo", FreebaseInfo.opts, "BridgeFn", BridgeFn.opts,
				"WordDistance", WordDistance.opts, "FormulaRetriever",
				FormulaRetriever.opts, "Lexicon", Lexicon.opts, });
		return parser;
	}
}
